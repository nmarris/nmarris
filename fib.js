...
if (intCount < 2 || intCount > 100){             //should be less than 2 since we have the 1st two numbers
    alert("error, Invalid number");
}
else {
    while (intCount > 2) {
        k = i + j;
        stringOutput = stringOutput + " " + k;    //need this statement to add Fib numbers
        i = j;
        j = k;
        intCount--;                              //need to decrement from the starting point until we reach 2   
    }
}
$("output").value = stringOutput;             // you had these backwards - removed your other statement
}; 
